...
type Tool = "audio" | "slides" | "video" | "mindmap" | "report" | "flashcards" | "quiz" | "infographic" | "datatable" | "social";
type Status = "DRAFT" | "SUBMITTED" | "UNDER_REVIEW" | "APPROVED";

const TOOLS: { id: Tool; label: string; icon: string; desc: string; badge?: string }[] = [
  { id: "social",      label: "Social Campaign",  icon: "", desc: "Multi-platform syndication", badge: "Automated" },
  { id: "audio",       label: "Audio Overview",  icon: "", desc: "Script for spoken summary" },
  ...
  { id: "flashcards",  label: "Flashcards",       icon: "",  desc: "Q&A revision cards", badge: "Education" },
      ...
      <div className="ml-auto w-full max-w-2xl h-full bg-white flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-shrink-0 p-4 border-b" style={{ background: "var(--primary-navy)", borderColor: "rgba(255,255,255,0.08)" }}>
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors text-white/50 hover:text-white" style={{}} onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.1)")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")} aria-label="Close Studio">x</button>
          </div>
          {/* Sources strip */}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              ...
              {sources.filter(s => s.type === "publication").length > 1 && (
                <div className="mt-3 p-3 rounded-lg text-[10px]" style={{ background: "#fffbeb", border: "1px solid #fef08a", color: "#78350f" }}>
                  <div className="font-semibold mb-1">Warning: Source review recommended</div>
            ...
            <div className="flex-shrink-0 p-4 border-t bg-slate-50 space-y-2" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {status === "UNDER_REVIEW" && (
                  <button className="btn-primary btn-sm" onClick={() => setStatus("APPROVED")}>Approve OK</button>
                )}
                {status === "APPROVED" && (
                  <span className="text-xs font-semibold text-green-700 bg-green-50 px-3 py-1.5 rounded-lg">OK Approved — ready to publish</span>
                )}
                ...
                <button
                  ...
                  style={{ background: "linear-gradient(135deg, #1e40af 0%, #2563eb 100%)", borderColor: "#1d4ed8" }}
                  onClick={() => {
                    onClose();
                    onOpenSocial?.(sources[0]?.title || "NCPOR Polar Research Dispatch", content);
                  }}
                >
